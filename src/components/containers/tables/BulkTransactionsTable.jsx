...
import { PAYMENT_PROTOCOL, TRANSACTION_STATUS_COLOR_MAP } from '@/lib/constants'
...
export default function BulkTransactionsTable({
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (
      ...
      Array.from(serviceProtocolFilter).length !== PAYMENT_PROTOCOL.length
    ) {
      filteredrows = filteredrows.filter((row) =>
        Array.from(serviceProtocolFilter).includes(row.status),
      )
    }
  ...
  console.log(serviceProtocolFilter)
        ...
        <div className="flex gap-3">
          ...
          <div className="relative flex gap-3">
            <SingleSelectionDropdown
              ...
              dropdownItems={PAYMENT_PROTOCOL}
