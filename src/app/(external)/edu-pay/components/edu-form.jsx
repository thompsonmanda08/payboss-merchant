...
export default function EduPaymentForm({}) {
  // Validation state
  ...
  const validateStep1 = useCallback(() => {
    ...
    if (!formData.payerDetails.fullName.trim())
      newErrors.fullName = "Full name is required";
    if (!formData.payerDetails.nrc.trim()) newErrors.nrc = "NRC is required";
    if (!formData.payerDetails.address.trim())
      newErrors.address = "Address is required";
    if (!formData.payerDetails.email.trim()) {
      newErrors.email = "Email is required";
    } else if (!validateEmail(formData.payerDetails.email)) {
      newErrors.email = "Please enter a valid email address";
    }
    if (!formData.payerDetails.phone.trim())
      newErrors.phone = "Phone number is required";

    setErrors(newErrors);

    return Object.keys(newErrors).length === 0;
  }, [formData.school, formData.payerDetails]);
